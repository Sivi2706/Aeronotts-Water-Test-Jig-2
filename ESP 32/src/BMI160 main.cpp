...
// Your BMI160 address (scan found 0x69)
static constexpr uint8_t BMI_ADDR = 0x69;

// Output rate (Jeff’s example feels good ~50–100Hz)
static constexpr float SAMPLE_HZ = 100.0f;
static constexpr float DT = 1.0f / SAMPLE_HZ;

// Mahony gains (tune these two)
static constexpr float TWO_KP = 2.0f * 1.2f;   // proportional
static constexpr float TWO_KI = 2.0f * 0.05f;  // integral (bias correction)
...
// Fast inverse sqrt (optional)
static inline float invSqrt(float x) {
  return 1.0f / sqrtf(x);
}

// Mahony update using accel + gyro (no magnetometer => yaw drift is normal)
static void mahonyUpdate(float gx, float gy, float gz, float ax, float ay, float az, float dt) {
  // Normalize accelerometer (must not be zero)
...
// Jeff-like quaternion -> gravity -> yaw/pitch/roll (degrees)
static void quatToYPR(float &yawDeg, float &pitchDeg, float &rollDeg) {
  // Gravity vector (same idea as Jeff's dmpGetGravity)
  float gx = 2.0f*(q1*q3 - q0*q2);
  float gy = 2.0f*(q0*q1 + q2*q3);
  float gz = q0*q0 - q1*q1 - q2*q2 + q3*q3;

  // Yaw/Pitch/Roll (matches common MPU6050 DMP convention)
...
void setup() {
  ...
  // Header for CSV
  Serial.println("DMPSTYLE,qw,qx,qy,qz,yaw,pitch,roll");
}
...
void loop() {
  ...
  // Output both quaternion + ypr (viewer can use either)
