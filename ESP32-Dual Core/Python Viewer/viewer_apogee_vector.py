...
# =========================
scene = canvas(
    title=f"Omnidirectional Apogee Viewer | scene UP = +Z | model yaw offset {MODEL_YAW_OFFSET_DEG:.0f}°",
    width=1250, height=760,
    background=vector(0.95, 0.95, 0.95)
)

# ---- FIX: prevent “model disappears” due to autoscale/zoom-out ----
scene.autoscale = False
scene.center = vector(0, 0, 0)
scene.range = 7  # increase if you want more zoom-out, decrease to zoom-in

# Simple “UP direction” label (default up = +Z)
up_lbl = label(
    pos=vector(0, 3.4, 0),
    text="UP = +Z (default)",
    height=14,
